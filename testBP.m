...
projectile = 8; %change this value to test different trajectory
projectile_start = start_point(projectile);
if projectile ~= length(start_point)
    projectile_end = start_point(projectile+1)-1;
else
...
% ***************** draw trajectories ********************
figure('Name','trajectories'), hold on
plot(p(2,:),p(3,:),'r x :', 'MarkerSize',10)
plot(p_whole(2,:),p_whole(3,:),'b o -', 'MarkerSize',10)
plot(trajectories{projectile}(1,:),trajectories{projectile}(2,:),'g * -.', 'MarkerSize',10)
...
figure('Name','predicted trajectory'),hold on; plot(p(2,:),p(3,:),'r x :', 'MarkerSize',10)
